...
#if UNITY_EDITOR
using UnityEditor; // 유니티 에디터에서만 나오는 로그
#endif
...
public class Enemy : LivingEntity
{
    ...
    public float attackRadius = 2f; // 좀비가 볼 수 있는 거리
    private float attackDistance; // 좀비가 돌아다니는 거리(Patrol 상태일 때)
    ...
    private void OnDrawGizmosSelected() // 이 함수가 소속된 스크립트가 컴포넌트로서 붙은 오브젝트가 선택됐을때만 보이도록 하는 이벤트 함수.
    {
        if (attackRoot != null)
        {
    ...
    {
        if (dead) return; // Zombie가 죽었다면 FixedUpdate 함수 종료
    ...
    // 데미지를 입었을때 실행할 처리
    public override bool ApplyDamage(DamageMessage damageMessage) // 데미지 처리
    {
    ...
    private bool IsTargetOnSight(Transform target)
    {
        ...
        // 1️ 그 광선이 시야각을 벗어나선 안되며
        if (Vector3.Angle(direction, eyeTransform.forward) > fieldOfView * 0.5f)
        {
            return false;
        }

        // 2️ 시야각 내에 존재 하더라도 광선이 장애물에 부딪치지 않고 목표에 잘 닿아야 함
