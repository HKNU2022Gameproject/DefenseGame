...
public class PlayerShooter : MonoBehaviour
{
    ...
    private void UpdateAimTarget() // aimPoint을 매 프레임마다 갱신
    {
        RaycastHit hit;
    ...
    private void OnAnimatorIK(int layerIndex)
    {
        ...
        playerAnimator.SetIKPosition(AvatarIKGoal.LeftHand,
            gun.leftHandMount.position); // 왼손을 총의 왼손 손잡이 부분을 중심으로 꺾어서 위치시킴
        playerAnimator.SetIKRotation(AvatarIKGoal.LeftHand,
            gun.leftHandMount.rotation); // 왼손을 총의 왼손 손잡이 부분을 중심으로 꺾어서 회전시킴
    }
}